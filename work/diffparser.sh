...
environments:
WORKSPACE:if  -i/--workspace is not specified, the tool will check the environment variable, and if the variable does not exist or is NULL, the tool will exist.
...
NICKNAME: if -n/--nickname 

example:
./changesetparser.sh -b pcmdev_PCM4_2_0_0_SNAPSHOT  -w xianwu_pcmdev_PCM4_2_0_Gold_Patch_Workspace -c pcmdev -n leo
_EOF
    exit 0
}
